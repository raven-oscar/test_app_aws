...
resource "aws_iam_role_policy_attachment" "ec2_cd_fullaccess_attach" {
  role       = aws_iam_role.devops_codedeploy_role.name
  policy_arn = var.AmazonEC2FullAccess_arn
}

resource "aws_iam_role_policy_attachment" "sns_cd_fullaccess_attach" {
  role       = aws_iam_role.devops_codedeploy_role.name
  policy_arn = var.AmazonSNSFullAccess_arn
}